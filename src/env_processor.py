...
class ENVProcessor:
        ...
        with self._lock:
            ...
            if os.path.exists(self.env_file_path):
                try:
                    ...
                    print(
                        f"Warning: Could not load credentials from {self.env_file_path}: {e}"
                    )
    ...
    def _remove_from_env_file(self, key: str):
        ...
        try:
            ...
            # Filter out the key
            filtered_lines = [
                line for line in lines if not line.strip().startswith(f"{key}=")
            ]
    ...
    def get_credentials_by_prefix(self, prefix: str) -> Dict[str, str]:
        ...
        return {
            key: value
            for key, value in self.available_credentials.items()
    ...
    def validate_required_credentials(self, required_keys: list) -> Dict[str, Any]:
        ...
        for key in required_keys:
            if (
                key in self.available_credentials
                and self.available_credentials[key].strip()
            ):
                present.append(key)
            else:
                missing.append(key)
    ...
    def export_credentials(
        self, keys: Optional[list] = None, format: str = "env"
    ) -> str:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "last_modified": (
                time.ctime(self._last_modified) if self._last_modified else "Never"
            ),
            "categories": {},
        }

        # Categorize credentials
        categories = {"api_keys": 0, "tokens": 0, "passwords": 0, "urls": 0, "other": 0}
