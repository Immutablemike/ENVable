...
class ENVSyncHandler(FileSystemEventHandler):
    ...
    def sync_secrets(self):
        ...
        creds_to_sync = {
            key: value for key, value in all_creds.items()
        ...
        print(f"🎯 Syncing {len(creds_to_sync)} credentials: {list(creds_to_sync.keys())}")
        ...
        for repo_name in self.config['repos']:
            ...
            try:
                ...
                for key, value in creds_to_sync.items():
                    ...
                    os.system(f'gh secret set {key} --body "{value}" --repo {owner}/{repo_name}')
